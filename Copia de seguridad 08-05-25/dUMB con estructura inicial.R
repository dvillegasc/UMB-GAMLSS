...
pUMB(q = 5 , mu = 0, sigma = 1, nu = 1, tau = 1, lower.tail = TRUE, log.p = FALSE)

qUMB(p=0.5, mu = 0, sigma = 1, nu = 1, tau = 1)

rUMB(n = 10, mu = 0, sigma = 1, nu = 1, tau = 1)
...
curve(dUMB(x, mu=7, sigma= 0.10, nu=0.05, tau=1),
      from=0, to=1, col="blue", las=1, ylab="f(x)", ylim = c(0,15), add=TRUE, )

curve(dUMB(x, mu=7, sigma= 0.25, nu=0.05, tau=1),
      from=0, to=1, col="red", las=1, ylab="f(x)", ylim = c(0,15), add=TRUE)

curve(dUMB(x, mu=7, sigma= 0.50, nu=0.05, tau=0.5), ylim = c(0,15),
      add=TRUE, col="green")

curve(dUMB(x, mu=7, sigma= 1, nu=0.05, tau=1),
      from=0, to=1, col="pink", las=1, ylab="f(x)", ylim = c(0,15), add=TRUE)

curve(dUMB(x, mu=7, sigma= 2, nu=0.05, tau=0.1), ylim = c(0,15),
      add=TRUE, col="black")

legend("topright", col=c("blue3", "red","green4", "pink", "black"), lty=1, bty="n",
       legend=c("mu=7, sigma=0.10, nu=0.05, tau=0.5",
                "mu=7, sigma=0.25, nu=0.05, tau=0.5",
                "mu=7, sigma=0.50, nu=0.05, tau=0.5",
                "mu=7, sigma=1.0, nu=0.05, tau=0.5",
                "mu=7, sigma=2.0, nu=0.05, tau=0.5"))
...
legend("topleft", col=c("black","green","pink", "red", "blue3"), lty=1, bty="n",
       legend=c("mu=7, sigma=0.10, nu=0.05, tau=0.5",
                "mu=7, sigma=0.25, nu=0.05, tau=0.5",
                "mu=7, sigma=0.50, nu=0.05, tau=0.5",
                "mu=7, sigma=0.70, nu=0.05, tau=0.5",
                "mu=7, sigma=0.9, nu=0.05, tau=0.5"))
